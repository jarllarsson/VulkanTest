...
namespace VulkanShaderLoader
{
	...
	VkPipelineShaderStageCreateInfo LoadShaderSPIRV(const std::string& in_fileName, const std::string& in_methodName, const VkDevice& in_device, VkShaderStageFlagBits in_stage)
	{
		...
		shaderStage.pName = in_methodName.c_str();
		if (shaderStage.module == NULL) throw ProgramError(std::string("Load SPIR-V shader: ") + in_fileName);
		return shaderStage;
	}

	VkPipelineShaderStageCreateInfo LoadShaderGLSL(const std::string& in_fileName, const std::string& in_methodName, const VkDevice& in_device, VkShaderStageFlagBits in_stage)
	{
		VkPipelineShaderStageCreateInfo shaderStage = {};
		shaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		shaderStage.stage = in_stage;
